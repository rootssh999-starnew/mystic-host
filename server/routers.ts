...
export const appRouter = router({
  ...
  admin: router({
    ...
    exportEgg: adminProcedure.input(z.object({ id: z.number().int().positive() })).query(async ({ input }) => { const row = (await listEggs()).find((egg) => egg.id === input.id); if (!row) throw new TRPCError({ code: "NOT_FOUND", message: "Egg not found" }); return { version: 1, egg: row }; }),
