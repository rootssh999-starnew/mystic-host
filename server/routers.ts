...
export const appRouter = router({
  ...
  node: router({
    ...
    updateSchedule: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => { const server = await requireServerPermission(ctx, input.name, "schedule.update"); const rows = await listSchedules(server.id); if (!rows.some(row => row.id === input.scheduleId)) throw new TRPCError({ code: "NOT_FOUND", message: "Schedule not found" }); return updateSchedule(input.scheduleId, { name: input.scheduleName, cron: input.cron, action: input.action, payload: input.payload }); }),
    deleteSchedule: protectedProcedure
      .input(z.object({ name: z.string().min(2).max(48), scheduleId: z.number().int().positive() }))
      .mutation(async ({ ctx, input }) => { const server = await requireServerPermission(ctx, input.name, "schedule.update"); const rows = await listSchedules(server.id); if (!rows.some(row => row.id === input.scheduleId)) throw new TRPCError({ code: "NOT_FOUND", message: "Schedule not found" }); await deleteSchedule(input.scheduleId); return { success: true }; }),
