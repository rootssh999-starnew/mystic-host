...
function FilesView({ onAction, serverName }: { onAction: (action: string) => void; serverName: string }) {
  const createFolderMutation = trpc.files.createFolder.useMutation({ onSuccess: (result) => onAction("Folder created: " + result.path), onError: (error) => onAction(error.message) });
  ...
  const writeTextMutation = trpc.files.writeText.useMutation({ onSuccess: () => { onAction("File saved to the live workspace"); }, onError: (error) => onAction(error.message) });
  ...
  const renameMutation = trpc.files.rename.useMutation({ onSuccess: () => { void utils.files.list.invalidate({ serverName }); onAction("File renamed on the live workspace"); }, onError: (error) => onAction(error.message) });
