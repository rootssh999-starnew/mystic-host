...
function FilesView({ onAction, serverName }: { onAction: (action: string) => void; serverName: string }) {
  ...
  const liveFilesQuery = trpc.node.listFiles.useQuery({ name: serverName });
...
function DatabasesView({ onAction, serverName }: { onAction: (action: string) => void; serverName: string }) {
  ...
  const create = trpc.node.createDatabase.useMutation({ onSuccess: result => { onAction("Database " + result.name + " created"); setName(""); setPassword(""); }, onError: e => onAction(e.message) });
...
export default function Home() {
  ...
  const renderPanel = () => {
    ...
    if (activePanel === "Startup") return <StartupView {...props} serverName={activeServer.name} startup="node server.js" onSave={(startup) => settingsMutation.mutate({ name: activeServer.name, startup })} onSaveVariables={(variablesJson) => settingsMutation.mutate({ name: activeServer.name, variablesJson })} />;
