...
function ActivityView({ serverName }: { serverName: string }) {
  const query = trpc.node.activity.useQuery({ name: serverName });
  const events = [...(query.data?.jobs ?? []).map((job) => ({ key: "job-" + job.id, event: job.type + " · " + job.status, when: job.updatedAt || job.createdAt, tone: job.status === "failed" ? "red" : job.status === "completed" ? "cyan" : "amber" })), ...(query.data?.runs ?? []).map((run) => ({ key: "run-" + run.id, event: "Schedule · " + run.scheduleName + " · " + run.status, when: run.finishedAt || run.startedAt, tone: run.status === "failed" ? "red" : run.status === "completed" ? "cyan" : "amber" }))].sort((a, b) => new Date(String(b.when)).getTime() - new Date(String(a.when)).getTime()).slice(0, 50);
  return <div className="subview"><div className="subview-heading"><div><span className="eyebrow">Audit trail</span><h2>Activity log</h2><p>Live jobs and schedule runs for this server.</p></div><button className="outline-button" onClick={() => query.refetch()}><RefreshCw size={14} />Refresh</button></div>{query.isLoading ? <div className="storage-loading">Loading activity…</div> : events.length === 0 ? <div className="empty-state"><div><History size={21} /></div><strong>No activity yet</strong><p>Server operations and scheduled actions will appear here.</p></div> : <div className="data-table activity-table"><div className="data-row data-head"><span>Event</span><span>Server</span><span>Actor</span><span>When</span><span /></div>{events.map((item) => <div className="data-row" key={item.key}><span className="activity-event"><span className={'activity-dot activity-dot-' + item.tone} />{item.event}</span><span className="muted-text">{serverName}</span><span className="muted-text">System</span><span className="muted-text">{new Date(String(item.when)).toLocaleString()}</span><ChevronRight size={15} className="muted-icon" /></div>)}</div>}</div>;
}
function HomeLogin() {
...
export default function Home() {
  ...
  const renderPanel = () => {
    ...
    return <ActivityView serverName={activeServer.name} />;
  };
