...
class SFTP {
  ...
  constructor(sftpStream, root) {
    this.root = path.resolve(root);

    flagsToString = (flags) => { if (flags & 0x10) return (flags & 0x01) ? "w+" : "w"; if (flags & 0x04) return (flags & 0x01) ? "a+" : "a"; if (flags & 0x02) return (flags & 0x01) ? "r+" : "r+"; return "r"; };
    SFTP_OPEN_MODE = { READ: 0x01, WRITE: 0x02 };
    SFTP_STATUS_CODE = { OK: 0, EOF: 1, NO_SUCH_FILE: 2, PERMISSION_DENIED: 3, FAILURE: 4 };
  ...
  pathRemoteToLocal(remotePath) {
    const relative = path.posix.normalize(String(remotePath || "/").replaceAll("\\", "/")).replace(/^\/+/, "");
    const local = path.resolve(this.root, relative);
    if (local !== this.root && !local.startsWith(this.root + path.sep)) throw new Error("Path escapes server volume");
    return local;
  }
  ...
  _realpath(reqid, filename) {
    logger.info('realpath ', filename, this.pathRemoteToLocal(filename));

    if(IS_WIN32)
      filename = pathLocalToRemote(this.pathRemoteToLocal(filename));
    else
